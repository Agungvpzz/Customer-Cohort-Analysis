...
class CohortPeriod(Cohort):
    def get_table(
        ...
        df["Period Number"
          ] = df['Period Date'].astype("int64") - df['Cohort'].astype("int64")
        
        df_pivot = df.pivot_table(
            index="Cohort",
            columns="Period Number",
            aggfunc={col_value: 'first'}
        )
    ...
    def get_pivot(
        ...
        df = df.sort_values(['Cohort', 'Period Date Step'])
        maps = df.groupby('Cohort').agg({'Period Date Step':
            'first'})['Period Date Step'].to_dict()
        df['Period Number'] = df['Period Date Step'].astype("int64") - df[
            'Cohort'].map(maps).astype("int64")
